...
def train(pretrained=False):
    ...
    seq_length = 64
    learning_rate = 10e-3
    model_path = f'saves_copy/model.pth'
    backup_path = f"saves_copy/model-b.pth"
    ...
    # Calculate and print model size (total number of parameters)
    ...
    output = model.generate(tokenizer, "Description Of Rama"
                            , num_tokens=50
                            , temperature=1.0
                            , top_k=None)
...
def my_gen(pretrained=False):
    ...
    # Load weights
    isLoaded, _, model, *_ = load_checkpoint(f'saves_copy/model.pth', model, None, None)
    if (not isLoaded):
        return

    # Generate text based on prompt
    output = model.generate(tokenizer, "Kaikeyi is one of the three queens"
                            , num_tokens=100
                            , temperature=0.8
                            , top_k=None)
